...
{
	void *root, *p1, *p2, VARIABLE_IS_NOT_USED *ref, *r1;
...
static bool test_misc(void)
{
	...
	char *p2;
	double *d;
	VARIABLE_IS_NOT_USED const char *name;
...
{
	void *root;
	VARIABLE_IS_NOT_USED struct el2 {
		const char *name;
	} *el2;
	struct el1 {
		int count;
...
static bool test_speed(void)
{
	...
	count = 0;
	do {
		void *p1, VARIABLE_IS_NOT_USED *p2, VARIABLE_IS_NOT_USED *p3;
		for (i=0;i<loop;i++) {
...
{
	void *root;
	char *p1, VARIABLE_IS_NOT_USED *p2;
	VARIABLE_IS_NOT_USED const char *name, *name2;
